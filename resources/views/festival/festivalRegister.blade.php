        ...
        <div class="w-[80vw] h-[75vh] bg-background_grey flex justify-center mx-auto mt-5 rounded-lg">
            <div class="flex flex-col space-y-5 p-5 w-full h-144"> <!-- Adjust height here -->
                <div class="flex justify-center flex-col space-y-10">
                    ...
                    <form action="{{ route('festival.register', ['festival' => $festival->id, 'user' => $user]) }}"
                        class="flex flex-col items-center space-y-3" method="POST">
                        @csrf
                        <p class="text-center">Do you want to cash in points, for every 500 pts you will get €10 off
                            your
                            order.<br> for every €1 spent you get 10 pts. Points can not be refunded</p>
                        <div>
                            <input type="number" name="points" id="points"
                                class="p-2 rounded-md justify-center w-24" placeholder="Pts" max="{{ $points }}"
                                min="0">
                            <select name="departure" id="">
                                @foreach ($busreizen as $busreis)
                                    <option value="{{ $busreis->departure }}">{{ $busreis->departure }}</option>
